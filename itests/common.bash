...
_profile() {
  ...
  memprofile=$(go tool pprof -normalize -top -show github.com/Nextdoor -ignore github.com/Nextdoor/pg-bifrost.git/stats -ignore github.com/klauspost -diff_base tests/$BATS_TEST_DESCRIPTION/perf_base/mem.pprof ../target/pg-bifrost tests/$BATS_TEST_DESCRIPTION/perf_output/mem.pprof | grep -v -E "^(\s+)?-" | tail -n +9 | tr -s ' ' | awk '{$1=$1};1' | cut -d ' ' -f5- | egrep -v vendor/gopkg.in/Nextdoor/cli)
  memory_leak=false
